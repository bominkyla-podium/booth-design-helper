...
st.caption("두꺼운 규정집 PDF를 올리면 AI가 핵심 일정과 시공 규정, 그리고 마감일이 강조된 타임라인 표까지 한눈에 요약해 줍니다.")
st.markdown("---")
...
if uploaded_file is not None:
    ...
    if st.button("🪄 규정 분석 시작하기"):
        with st.spinner("🤖 AI가 규정집을 분석하며 마감일을 강조하는 중입니다... 잠시만 기다려주세요."):
            try:
                client = genai.Client(api_key=GEMINI_API_KEY)
                
                # 💡 타임라인 표 내 마감일 강조 규칙 추가
                prompt = f"""
                ...
                4. 🚨 **[마감일 강조 규칙]**: 타임라인 표를 만들 때 '신고 마감', '신청 마감', '등록 마감', '제출 마감' 등 PM이 서류나 비용을 지불해야 하는 **주요 마감일 항목은 반드시 해야 할 일 칸의 글씨를 볼드체(**글씨**)로 쓰고, 항목 맨 앞에 '🔥' 아이콘을 붙여서 강력하게 강조**하세요. (일반 설치/철거 일정은 강조하지 마세요)
                ...
                *규정집에 명시된 모든 마감일과 공사 일정을 시간 순서대로 정리한 표입니다. (🔥 표시된 마감일은 필수 체크)*
                
                | 날짜 (Date) | 시간 (Time) | 해야 할 일 / 주요 일정 (Task) | 비고 및 중요도 (Notice) |
                | :--- | :--- | :--- | :--- |
                | 예: 2026-07-16 | ~ 18:00까지 | 🔥 **부스 활동 및 프로모션 풍선 신청 마감** | 마감 엄수 (추가 비용 발생) |
